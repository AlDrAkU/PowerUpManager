...
{
	// TODO: Uncomment next call of TestWithoutDrawing
	// It tests the constructor, AddItem, Size and destructor 
	TestWithoutDrawing( );

	// TODO: Uncomment next call of InitTestWithDrawing
	InitTestWithDrawing( );
}
...
{
	std::cout << "Test Draw, Update and HitItem :\n";
...
void Game::TestWithoutDrawing( )
{
	...
	Point2f center{};
	float dx{ PowerUp::radius * 2 };
	float dy{ PowerUp::radius * 2 };
...
void Game::TestAddingItems( PowerUpManager* pManager, const std::vector<Point2f>& centers, const std::vector< PowerUp::Type >& types )
{
	...
	{
		pManager->AddItem( centers[idx], types[idx] );
		++size;
		TestSize( pManager, size );
	}
}
...
{
	// Hit all the items
	Rectf destroyer{ 0,0,PowerUp::radius / 2,PowerUp::radius / 2 };

	for ( Point2f center : centers )
	{
		// Make destroyer hit teh item
...
void Game::TestHittingCircle(  std::vector<Point2f>& centers, const Rectf& destroyer )
{
	for ( Point2f& center : centers )
	{
		Circlef powerUp{ center, PowerUp::radius };
...
void Game::DrawCircles( const std::vector<Point2f>& centers, const std::vector< PowerUp::Type >& types ) const
{
	for ( size_t idx{ 0 }; idx < centers.size( ) ; ++idx )
	{
			...
			utils::FillEllipse( m_Centers[idx], PowerUp::radius, PowerUp::radius );
	}
}
